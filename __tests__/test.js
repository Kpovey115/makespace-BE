...
describe("/api/listings", () => {
    describe("GET ALL LISTINGS", () => {
        ...
        it("Status: 200, Responds with an array of filtered listing objects for multiple queries", () => {
            return request(app)
                .get(
                    "/api/listings?location.city=Manchester&amenities.kitchen=false"
                )
                .expect(200)
                .then(({ body }) => {
                    const { listings } = body;
            ...
            it("Status: 201. Responds with a listing object with the relevant properties", () => {
                ...
                return request(app)
                    ...
                    .then(({ body }) => {
                        ...
                        expect(listing.location.city).to.deep.equal(
                            "Liverpool"
                        );
                        expect(
                            listing.contactDetails.emailAddress
                        ).to.deep.equal("hollymay@gmail.com");
                    });
            });
            it("Status: 201. Responds with a listing object with the relevant properties", () => {
                const newListing = {
                ...
                return request(app)
                    ...
                    .then(({ body }) => {
                        ...
                        expect(
                            listing.contactDetails.emailAddress
                        ).to.deep.equal("Scarlett@gmail.com");
                    });
            });
        });
